...
class Producto
{
    ...
    private static function getProductsOfBd($order = 'SELECT * FROM productos')
    {
        try
        {
            $pdo = AccesoDatos::obtenerObjetoAcceso();
            $judgmen = $pdo->retornarConsulta($order);
            $judgmen->execute();
    ...
    public static function showListProduct($order = 0)
    {
        $ret = "LISTA DE PRODUCTOS:<br>";
        if($order)
        {
            $listProduct = self::getProductsOfBd('SELECT * FROM productos ORDER BY nombre DESC');
        }
        else
        {
            $listProduct = self::getProductsOfBd('SELECT * FROM productos ORDER BY nombre ASC');
        }
